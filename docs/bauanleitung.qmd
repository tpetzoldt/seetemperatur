...
Im Folgenden wird eine Messsonde beschrieben, mit der die Temperaturschichtung in einem See oder Teich gemessen werden kann. Die Temperaturschichtung beeinflusst die Sauerstoffversorgung in tieferen Schichten (Hypolimnion) und die Nährstoffverteilung.
...
Im Projekt sollen unterschiedliche Fähigkeiten geschult werden: Programmieren, etwas Elektronik, handwerkliches Geschick und am Ende natürlich die Gewässerökologie.
...
Andererseits lassen sich nicht alle Klippen vermeiden: Einrichten der Entwicklungsumgebung, Umgang mit dem Lötkolben, Bau eines einfachen Gehäuses.
...
* optional: Multimeter
* optional: 47Ω Widerstand (mindestens 0,5 W) für stabilere Stromversorgung
...
Da man für das Steckbrett einen MCmit Stiftleisten benötigt, für die endgültige Schaltung jedoch ohne, empfiehlt es sich, einen Dreierpack Mikrocontroller zu kaufen: einen für den Steckbrettaufbau, einen für die endgültige Schaltung und einen als Reserve.

Im Prinzip lassen sich auch andere MCs verwenden, allerdings benötigt man dann noch ein separates Display und hat etwas mehr Lötarbeit.
...
2. Eine Programmierumgebung zur Erstellung dieses Programms und um es auf den MC zu schreiben.
...
Die Details dazu sind auf diversen Internetseiten zu finden. Bei manchen Online-Händlern ist für den Mikrocontroller eine passende Anleitung oder ein E-Book erhältlich, in dem die Vorgehensweise noch genauer beschrieben ist.
...
Für den Steckbrettaufbau benötigen wir den mit Pinleisten verlöteten MC, drei verschiedenfarbige Kabel, einen Temperatursensor und einen Widerstand.

Wichtig ist, dass die Stifte des MC eine gute elektrische Verbindung haben. Deshalb ist zu empfehlen, die Stifte 
...
Bei einem Sensor mit Kabel muss man zunächst ebenfalls eine Stiftleiste mit 3 Pins anlöten.
...
Damit die Signalleitung einen definierten Wert hat, wird zwischen Signalleitung und Pluspol ein sogenannter Pullup-Widerstand von standardmäßig 4,7kΩ geschaltet. Das WiFi Kit besitzt auch interne Pullup-Widerstände, deshalb funktioniert es auf dem Steckbrett zuweilen auch ohne die 4,7kΩ. Für den Tiefensensor ist der 4,7kΩ-Widerstand unbedingt nötig, da sonst wegen des längeren Kabels Messaussetzer auftreten können.
...
Das Display sollte jetzt einen Temperaturwert anzeigen. 
...
**Vorgehensweise Serieller Monitor**

1. Stelle sicher, dass das Heltec Board über das USB-Kabel mit dem Computer verbunden ist.
2. Klicke in der Arduino IDE oben rechts auf das Lupen-Symbol (Serieller Monitor).
3. Wähle in der Dropdown-Liste des Seriellen Monitors die korrekte Baudrate aus. Sie muss mit der im Sketch festgelegten Rate übereinstimmen, aktuell 9600.
...
* Interne Datenloggerfunktion des Mikrocontrollers (falls dazu Bedarf besteht, kontaktiere uns bitte für weitere Informationen).
...
Stand 2026-05-17, Homepage des Autors: [https://tu-dresden.de/Members/thomas.petzoldt](https://tu-dresden.de/Members/thomas.petzoldt)
